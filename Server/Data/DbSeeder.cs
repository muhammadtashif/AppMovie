...
public class DbSeeder
{
    public static async Task SeedData(IMongoDatabase database)
    {
        ...
        var count = await moviesCollection.CountDocumentsAsync(FilterDefinition<Movie>.Empty);
        if (count > 0)
        {
            // Fix for Forrest Gump image if it exists with broken URL
            var updateFilter = Builders<Movie>.Filter.Eq(m => m.Title, "Forrest Gump");
            var update = Builders<Movie>.Update.Set(m => m.ImageUrl, "https://images.unsplash.com/photo-1533488765986-dfa2a9939acd?w=400");
            await moviesCollection.UpdateOneAsync(updateFilter, update);
        ...
        var movies = new List<Movie>
        {
            ...
            new Movie
            {
                ...
                Genre = "Drama",
                ReleaseYear = 1994,
                ImageUrl = "https://images.unsplash.com/photo-1533488765986-dfa2a9939acd?w=400"
            },
            new Movie
            {
                Title = "Avatar",
